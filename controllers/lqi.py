...
class Controller(BaseController):
  ...
  def update(self, target_lataccel, current_lataccel, state, future_plan):
    ...
    self.roll_ff = self.alpha_roll * self.roll_ff + (1.0 - self.alpha_roll) * roll0

    r = float(target_lataccel + self.roll_ff)  # reference
    ...
    # LQI control (K is 1x3 from augmented design)
    u = float(-(self.K @ z))  # no extra FF needed; integrator handles bias
