...
def run_rollout(data_path, controller_type, model_path,debug=False, ALPHA=0.9, KP=0.15, KI=0.01, KD=0.1,B=2.0, C=0.11,dt = 1.0):
    ...
    controller = controller_cls(alpha=ALPHA, p=KP, i=KI, d=KD,b=B, c=C, dt=dt)
...
if __name__ == "__main__":
  ...
  if data_path.is_file():
      grid_pid = {
          'alpha': [0.7, 0.8, 0.9, 0.95, 0.98, 1.0],
          'p': [0.2, 0.25, 0.3],
          'i': [0.01, 0.05, 0.1],
          'd': [0.1, 0.2,0.22],
          'b': [1.4,1.5,1.6,1.7,2.0, 2.5],
          'c': [0.0, 0.05, 0.1, 0.11]
      }

      results = []

      for alpha in grid_pid['alpha']:
          for p in grid_pid['p']:
              for i in grid_pid['i']:
                  for d in grid_pid['d']:
                      for b in grid_pid['b']:
                          for c in grid_pid['c']:
                              ...
                              print(f"α={alpha:.2f}, p={p:.2f}, i={i:.2f}, d={d:.2f}, "
                                    f"b={b:.2f}, c={c:.2f}  "
      ...
      print("\nBEST CONFIG:")
      print(f"α={best[0]:.2f}, p={best[1]:.2f}, i={best[2]:.2f}, d={best[3]:.2f}, "
            f"b={best[4]:.2f}, c={best[5]:.2f}  "
